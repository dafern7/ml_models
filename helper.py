...
def find_first_error(dataframe, replsolar):
    ...
    start = 0
    n = len(dataframe)
    bools = np.equal(np.equal(months[start:n],replsolar_months[start:n]),np.equal(hours[start:n],replsolar_hours[start:n]),np.equal(days[start:n],replsolar_days[start:n]))
...
def remove_solar_values(dataframe, FirstFalse,replicated_solar,start):
    ...
    boolsindex = list((replicated_solar.month == startmo) & (replicated_solar.day == startday) & (replicated_solar.hour==starthr))
...
def use_peak_shave_model(df,solar,hourly_intervals,tariff_id,power,capacity,eff=0.9,itc=True,project_type='solar+storage',export=False):
    """
    ...
    return relevant_data5, modelled
...
def annual_savings(relevant_data5,frac_of_year=1, verbose=False, project_type='solar+storage'):
    ...
    if verbose:
        if project_type == 'solar+storage':
            ...
            print('Estimated Energy Savings per year: ${:0.2f}'.format(energy_savings_total*(1/frac_of_year)), 'Solar portion: ${:0.2f}'.format(energy_savings_solar*(1/frac_of_year)),
    ...
    if (dem_cost_storage+dem_cost_after_solar)==0:
        demand_savings_total=0
        demand_savings_solar=0
        demand_savings_storage=0
        percent_dem_savings=0
    else:
    ...
    if verbose:
        if project_type == 'solar+storage':            
            ...
            print('Estimated Demand Savings per year: ${:0.2f}'.format(demand_savings_total*(1/frac_of_year)), 'Solar portion: ${:0.2f}'.format(demand_savings_solar*(1/frac_of_year)),
    ...
    if verbose:
        if project_type == 'solar+storage':
            ...
            print('Estimated Total Cost After Solar+Storage: ${:0.2f}'.format(general_cost_after_all*(1/frac_of_year)))
            print('Estimated Total Savings per year: ${:0.2f}'.format(general_savings_total*(1/frac_of_year)), 'Solar portion: ${:0.2f}'.format(general_savings_solar*(1/frac_of_year)), 
                  'Storage portion: ${:0.2f}'.format(general_savings_storage*(1/frac_of_year)))
            print("Percent Total Savings: {:0.2f}%".format(percent_gen_saving), 'Solar portion: {:0.2f}%'.format(percent_solar_savings),
                  'Storage Portion: {:0.2f}%'.format(percent_storage_savings))
        else:
            print("Estimated Building Total Costs: ${:0.2f}".format(general_building_cost*(1/frac_of_year)))
            print('Estimated Total Cost After Storage Only: ${:0.2f}'.format(general_cost_after_all*(1/frac_of_year)))
...
def generate_replicated_months(mnth,hourly_intervals,freq):
    ...
    datelist = pd.date_range((datetime.datetime(mnth.date.dt.year[0],mnth.date.dt.month[0],1,0,0,0)), periods=num_intervals, freq=freq).tolist()   
...
def generate_replicated_year(df,hourly_intervals,freq,leap_year=False):
    ...
    datelist = pd.date_range((datetime.datetime(df.date.dt.year[0],1,1,0,0,0)), periods=num_intervals, freq=freq).tolist()   
...
def bargraph_ready(df,solar,hourly_intervals,freq,tariff_id,power,capacity,eff,itc,project_type,export):
    ...
    for month_num in set(df.date.dt.month):
        ...
        month_peak_shave, modelled = use_peak_shave_model(month_repl,solar,hourly_intervals=hourly_intervals,tariff_id=tariff_id,power=power,capacity=capacity,
